...
if (context.media[0].path) {

  // Postman upload
  imagePath =
    context.media[0].path;

} else if (context.media[0].url) {
...
if (med.suggested) {
  ...
  session.data.pendingSuggestion = med;
  ...
  return [
    ...
    'Possible Match:',
    '',
    `1. ${med.suggested} (${med.confidence}%)`,
    '',
    '1 Accept Match',
...
session.data.items.push({
  ...
  controlledDrug: med.controlledDrug
});
